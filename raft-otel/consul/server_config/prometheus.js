  ...
  var prometheusProvider = {
    ...
    reformatSeries: function (unitSuffix, labelMap) {
      return function (response) {
        ...
        if (!response.data || !response.data.result || response.data.result.length == 0 || !response.data.result[0].values || response.data.result[0].values.length == 0) {
          return emptySeries;
        }
    ...
    fetchRPS: function (serviceName, type, options) {
      ...
      return this.fetchStat(this.groupQueryHTTP(type, q), "RPS", `<b>${subject}</b> request rate averaged over the last 15 minutes`, shortNumStr, this.groupBy(type));
    },
    ...
    fetchPercentile: function (percentile, serviceName, type, options) {
      ...
      return this.fetchStat(q, `P${percentile}`, `<b>${subject}</b> ${percentile}th percentile request service time over the last 15 minutes`, shortTimeStr, this.groupBy(type));
    },
    ...
    fetchConnRate: function (serviceName, type, options) {
      ...
      return this.fetchStat(this.groupQueryTCP(type, q), "CR", `<b>${subject}</b> inbound TCP connections per second averaged over the last 15 minutes`, shortNumStr, this.groupBy(type));
    },
    ...
    fetchServiceRx: function (serviceName, type, options) {
      ...
      return this.fetchStat(this.groupQueryTCP(type, q), "RX", `<b>${subject}</b> received bits per second averaged over the last 15 minutes`, shortNumStr, this.groupBy(type));
    },
    ...
    fetchServiceTx: function (serviceName, type, options) {
      ...
      var self = this;
      return this.fetchStat(this.groupQueryTCP(type, q), "TX", `<b>${subject}</b> transmitted bits per second averaged over the last 15 minutes`, shortNumStr, this.groupBy(type));
    },
    ...
    fetchServiceNoRoute: function (serviceName, type, options) {
      ...
      return this.fetchStat(this.groupQueryTCP(type, q), "NR", `<b>${subject}</b> unroutable (failed) connections per second averaged over the last 15 minutes`, shortNumStr, this.groupBy(type));
    },
